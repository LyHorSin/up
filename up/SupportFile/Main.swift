...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    public let parseServer = ESParseServerManager.share
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        parseServer.configuration(deviceToken: deviceToken)
        firebaseManager.saveFCMToken(deviceToken: deviceToken)
    }
    
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        PFPush.handle(userInfo)
        print("Received Push Notification with data:", userInfo)  // Printing the notification data
